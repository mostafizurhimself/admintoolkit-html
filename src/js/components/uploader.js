...
const uploader = {
  init() {
    ...
    if(dropzoneMultiple.length) {
      [...dropzoneMultiple].forEach(dropzone => new Dropzone(dropzone, {
        ...defaultOptions,
        hiddenInputContainer: dropzone
      }))
    }
    ...
    if(dropzoneDisabled.length) {
      [...dropzoneDisabled].forEach(dropzone => new Dropzone(dropzone, {
        ...defaultOptions,
        clickable: false,
        hiddenInputContainer: dropzone
      }))
    ...
    if(dropzoneCustom.length) {
      [...dropzoneCustom].forEach(dropzone => new Dropzone(dropzone, {
       ...defaultOptions,
        maxFiles: 1,
        hiddenInputContainer: dropzone,
      }))
